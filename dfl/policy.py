...
from simulator import getTopk
from config import dim_dict, policy_map
from dflutils import getSoftTopk, DiffTopK, nck
...
def get_trajectory_probability(states, actions, w_train,k,epsilon,ntr, T):
    bs, N = states.shape
    default_indices = tf.tile(tf.reshape(tf.range(N), (1, N)), (bs, 1))
    states = tf.cast(states, dtype=tf.int32)
    actions = tf.cast(actions,dtype=tf.int32)
    states_with_indices = tf.concat([tf.expand_dims(default_indices, axis=-1), tf.expand_dims(states, axis=-1)],
                                    axis=-1)

    w_selected = tf.gather_nd(w_train, states_with_indices)

    diffTopK = DiffTopK(k=k, epsilon=epsilon)
    gamma = diffTopK(-w_selected)
    probs = gamma[:, :, 0] * N
    H = bs//ntr
    probs_reshaped = tf.reshape(probs, (ntr, H,N))
    states = tf.reshape(states, (ntr, H, N))
    T = tf.cast(T, dtype=float)
    probs = 0
    for i in range(ntr):
        for j in range(N):
            for k in range(H-2):
                if actions[i,k,j] == 1:
                    probs += tf.math.log(probs_reshaped[i,k,j])
#                     probs += tf.math.log((T[j,states[i,k,j], actions[i,k,j], states[i,k+1,j]]))
                else:
                    probs += tf.math.log(1-probs_reshaped[i, k, j])
#                     probs += tf.math.log((T[j, states[i, k, j], actions[i, k, j], states[i, k + 1, j]]))
...
def getProbs(states, policy, ts, w, k, epsilon=0.1):
    ...
    if policy == 0:
        ...
        gamma = diffTopK(-w_selected)
        probs = gamma[:,:,0]
        selection = np.zeros((bs,N),dtype=int)
        # for i in range(bs):
        #     normalized_probs = probs[i].numpy()/np.linalg.norm(probs[i], ord=1)
        #     # normalized_probs[-1] = 1 - np.sum(normalized_probs[0:-1])
        #     selected = np.random.choice(a=states[i], p=normalized_probs, size=k, replace=False)
        #     for j in selected:
        #         selection[i,j] = 1

        # print('probs shape', probs.shape)
    return probs * N, w_selected
