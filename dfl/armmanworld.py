...
from synthetic import genProbabilities,genProbabilitiesBigMDP
from content_reward_simulate import step
...
class ArmmanWorld:
    ...
    def __init__(self, size,k,H,seed=0):
        self.size = size
        self.k=k
        self.H = H
        self.horizon = H
        self.n_actions = int(math.factorial(size)/(math.factorial(size-k)*math.factorial(k)))

        self.n_states = 2**size
        self.small_p_transition = genProbabilities(self.size,seed)
        self.p_transition = genProbabilitiesBigMDP(self.size, self.n_states,self.n_actions,k,seed=0,P=self.small_p_transition)
    ...
    def state_index_transition(self, s, a):
        ...
        """
        # return step(s,a,self.p_transition)
        return NotImplementedError
    ...
    def __repr__(self):
        return "ArmmanWorld(size={})".format(self.size)
