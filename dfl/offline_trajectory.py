...
def get_offline_dataset(policy, T, seed=0):
    ...
    for idx in range(n_instance):
        ...
        print(ope_simulator_subset.emp_T_data.shape)
        T_data_list.append(ope_simulator_subset.emp_T_data)
    ...
    all_T_data = np.concatenate(T_data_list, axis=0)
    with open('T_data.obj', 'wb') as f:
        pickle.dump(all_T_data, f)
